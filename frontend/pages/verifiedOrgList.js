...
export default function verifiedOrgList() {
    ...
    const getSetOfVerifiedOrgs = async () => {
        try {
            const signer = await getProviderOrSigner(true);
            const regContract = new Contract(REGISTER_CONTRACT_ADDRESS, abi, signer);
            const verifiedOrgs = await regContract.verifiedOrganizationsList();
    ...
    return (
        <div>
            <div className=' bg-black '>
                <button className="bg-gray-900" onClick={getSetOfVerifiedOrgs}>
                    Reg Org.
                </button>
