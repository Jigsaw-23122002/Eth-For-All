...
export default function reportedOrg() {
    ...
    const upVoteOrg = async () => {
        ...
        const valUpVotes = await voteContract.violationUpVote(query.id);
        await valUpVotes.wait();
        const val = await voteContract.violationUpVoteCount(query.id)
        // await val.wait(2);
    ...
    const downVoteOrg = async () => {
        ...
        const signer = await getProviderOrSigner(true);
        const voteContract = new Contract(REGISTER_CONTRACT_ADDRESS, abi, signer);
        const valDownVotes = await voteContract.violationDownVote(query.id);
        await valDownVotes.wait();
        const val = await voteContract.violationDownVoteCount(query.id)
        // await val.wait(2);
        console.log(val._isBigNumber);
        console.log(val.toString());
    ...
    const setViolationstatus = async () => {
        const today = new Date();
        const timeNow = Math.floor((today.getTime()) / 1000);
        const signer = await getProviderOrSigner(true);
        const voteContract = new Contract(REGISTER_CONTRACT_ADDRESS, abi, signer);
        const valDownVotes = await voteContract.setViolationStatus(query.id);
        await valDownVotes.wait();
        const val = await voteContract.violationDownVoteCount(query.id)
        // await val.wait(2);
        // console.log(val._isBigNumber);
            ...
            <div className='p-6 grid grid-cols-4'>
                ...
                <div className=" m-6 w-4/5 h-3/4 p-6 grid grid-rows-6 bg-transparent">
                    ...
                    <div className='row-span-4 grid grid-rows-4 w-full items-center border border-gray-200  bg-transparent rounded-lg '>
                        ...
                        <div className='px-6  text-gray-100 grid grid-cols-2'>
                            <p className='h-6 text-lg'>{query.views}</p>
