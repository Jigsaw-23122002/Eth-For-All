...
export default function orgDetails() {
  ...
  const [verifiedOrgDetails, setverifiedOrgDetails] = useState({
    pic_cid: "bafybeidy6jb3q4etakqsgizfnp2pp7novz6i457lwq5l3kg2stp5kcof7i",
    details: [
      ...
      " That’s why we made the decision to donate our wealth from Microsoft to help others.The challenge when we started out was how to do that in a meaningful and high-impact way. We were drawn to things that sprang from our experience, so we began donating PCs to public libraries across the United States to give everyone a chance to use one.",
  ...
  const getTransactionDetails = async () => {
    try {
      const result = await axios.post(
        "https://api.studio.thegraph.com/query/41847/test/0.69.042",
        {
          query: `query MyQuery {
              transfers {
                from
  ...
  const send = () => {
    console.log(val);
    let ethersToWei = ethers.utils.parseUnits(val, "ether");
  ...
  return (
    <div className="bg-black p-16">
        ...
        <div className="space-y-6 md:space-y-0 md:flex md:gap-6 lg:items-center lg:gap-12">
          <div className="md:5/12 lg:w-5/12">
            <img
              ...
              src={`https://${verifiedOrgDetails.pic_cid}.ipfs.w3s.link/${fileName}`}
              alt="image"
              loading="lazy"
              width=""
              height=""
          ...
          <div className="md:7/12 lg:w-6/12">
            <h2 className="text-3xl font-extrabold pt-5 text-white text-center">
              {query.name}{" "}
            </h2>
            <hr className="w-1/4 h-1 mx-auto  bg-gray-600 border-0 rounded mt-2 mb-5 dark:bg-gray-700"></hr>
            <div className="mt-6 text-gray-100 ">
              {verifiedOrgDetails.details.map((ele) => {
                return (
                  <div key={ele} className="text-center">
                    &emsp; {ele}
                  </div>
                );
              })}
      ...
      <div
        id="drawer-contact"
        className="fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-black w-80 dark:bg-gray-800"
        tabindex="-1"
      ...
      <div>
        <div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
          <div className="inline-block min-w-full shadow rounded-lg overflow-hidden">
            <table className="min-w-full leading-normal">
              <thead>
                <tr>
                  {tableTitle.map((ele) => {
                    return (
                      <th className="px-5 py-3 border-b-2 border-gray-800 bg-gray-700 text-left text-xs font-semibold text-gray-200 uppercase tracking-wider">
                        {ele}
                      </th>
                    );
                  })}
            ...
            <div className="px-5 py-5 border-gray-900 bg-gray-700 border-t flex flex-col xs:flex-row items-center xs:justify-between          ">
